...
const SwarderPage = () => {
  ...
  const filtered = products
    // 1. Force array to only include Swarder specific categories
    .filter(p => swarderCategories.includes(p.category))
    // 2. Filter by user selected category
      ...
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-6">
          ...
          <div className="bg-white rounded-[3rem] p-12 lg:p-20 tech-shadow mb-12 relative overflow-hidden border border-slate-100/50">
            <div className="absolute top-0 left-0 w-64 h-64 bg-green-500/10 rounded-full blur-3xl -ml-32 -mt-32" />
            <div className="relative z-10 text-center lg:text-right max-w-3xl space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 text-green-600 text-[10px] font-black uppercase tracking-widest border border-green-500/20">
                <Box className="w-3 h-3" />
                Akıllı Ulaşım Bölümü
              </div>
              ...
              <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-xl lg:mr-0 lg:ml-auto">
                Üstün kalite garantisi ile en yeni Elektrikli Bisikletleri ve Scooterları keşfedin. Akıllıca hareket edin ve özgürlüğün tadını çıkarın.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <aside className="lg:w-80 space-y-10">
              ...
              <div className="space-y-4">
                <h3 className="text-slate-900 font-black text-sm uppercase tracking-widest">Kategoriler</h3>
                <div className="flex flex-col gap-2">
                  {categories.map((cat) => (
                    <button
                      key={cat.id}
                      onClick={() => setActiveCategory(cat.id)}
                      className={`flex items-center justify-between px-6 py-4 rounded-xl font-bold transition-all ${
                    ...
                    >
                      <span>{cat.label}</span>
                      <div className={`w-2 h-2 rounded-full ${activeCategory === cat.id ? "bg-white animate-pulse" : "bg-slate-200"}`} />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex-1 space-y-8">
              ...
              {filtered.length > 0 ? (
                ...
                <div className="bg-white rounded-[3rem] py-32 tech-shadow flex flex-col items-center justify-center space-y-6">
                  ...
                  <Button onClick={() => {setSearchQuery(""); setActiveCategory("all")}} variant="outline" className="rounded-xl h-12 font-bold px-8">
                    Aramayı Sıfırla
                  </Button>
                </div>
              )}
